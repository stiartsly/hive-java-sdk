...
public class AuthHelper implements ConnectHelper {
	...
	private AuthPersistent persistent;

	private ConnectionManager connectionManager;
	private AppContextProvider context;
	private AuthenticationAdapter authenticationAdapter;

	public AuthHelper(AppContextProvider context, String ownerDid, String endPoint, AuthenticationAdapter adapter) {
	...
	private void signIn() throws HiveException {
		...
		try {
			...
			JsonNode ret = HttpUtil.getValue(response, JsonNode.class).get("challenge");
			if(null == ret)
	...
	private void handleAuthResponse(Response response) throws Exception {
		JsonNode ret = HttpUtil.getValue(response, JsonNode.class).get("access_token");
		if(null == ret)
			throw new HiveException("Sign in failed");
	...
	public AppContextProvider getContext() {
		return this.context;
	}
